            ...
            {!findHidden ? (
                <Container className={classes.help}>
                    <Paper className={classes.helpPage}>
                        ...
                        <p>Once a spreadsheet is created on <a href='https://www.google.com/sheets/about/'>Google Sheets</a>, give a title the spreadsheet and sheets using camel case (for example: camelCasing).  Please refrain from using special characters for the sheet title in Google Sheets as some will cause errors.  However, once the spreadsheet has been retrieved authorized accounts will be able to modify the titles.</p>
                    </Paper>
                    <Paper className={classes.helpPage}>
                        <h3>Spreadsheet Format</h3>
                        <p>The list of lines consist of two columns. Row's 1 and 10 are designated as header rows. Row 1 consist of two columns, line(A) and number(B).  As row 10 has five columns, name(A), phone tag(B), extension(C), room(D), and note(E).</p>
                    </Paper>
                </Container>
            ) : null }
        </Grid>
